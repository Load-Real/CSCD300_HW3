public class MyLinkedList {
	...
	private MyLinkedList reverse(ListNode node) {
		//In case the LinkedList is empty or the end has been reached
      if (node == null) return new MyLinkedList();
      
      MyLinkedList reversedList = reverse(node.next);
      //Need to add last instead, otherwise I just return an identical LinkedList
      reversedList.addLast(node.data);
   ...
   public void addLast(Object data) {
	...
	private ListNode reverse(ListNode first, ListNode second) {
      ...
      //Second being null signifies that the list ends at where first is
      if (second == null) return first; //base case
      else {
         //Our new head node is equal to the recursive call passing the next two values (which will be swapped around)
         newHead = reverse(second, second.next);
         //Second points to first (signifying the list is being reversed)
         second.next = first;
         //Since first would be at the end of the list, it makes sense that .next is null
         first.next = null;
      }
		
      //Return our new updated node
