...
public class Tester {
	...
	public int countSpace(String str) {	
      ...
      //char to temporarily store current character
      char currentChar = str.charAt(0);
      if (currentChar == ' ') {
         return countSpace(str.substring(1)) + 1; //recursive call with increment
      }
      
		return countSpace(str.substring(1)); //recursive call if char != ' '
	}
	...
	public boolean myContains(String s1, String s2){
      ...
      //Condition to check if a substring is in the method
	...
	private boolean isSum24(int arr[], int targetSum) {
      ...
      if (arr.length == 1 && (targetSum - arr[0]) == 0) return true;
   
      int[] shrunkArr = new int[arr.length - 1];
      for (int i = 0; i < arr.length - 1; i++) {
         shrunkArr[i] = arr[i + 1];
      }
   
		return isSum24(shrunkArr, targetSum - arr[0]); //change this line of code as needed.
	}
	...
	private void reverseArray(int a[], int low, int high) {
      if (low < high) {
         //Perform a swap here to move the values at the low and high indices around
         //Stops when low and high meet in the middle
         int temp = a[low];
         a[low] = a[high];
         a[high] = temp;
         
         //Raise the index of low and lower the index of high until they meet in the middle of the array
         //Use prefix operation here to prevent StackOverFlow
